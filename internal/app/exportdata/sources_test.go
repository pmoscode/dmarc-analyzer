...
func TestWriteSourceStatsCSV_HeaderAndRows(t *testing.T) {
	...
	err = exportdata.WriteSourceStatsCSV(&buf, []sources.Stat{{
		SourceIP: ip, TotalCount: 42, PassRate: 0.5,
	...
	out := buf.String()
	require.Contains(t, out, "SourceIP,TotalCount,PassRate,Hostname,Service,FirstSeen,LastSeen")
	require.Contains(t, out, "203.0.113.1,42,0.5000,mail.example.com,Beispieldienst")
}

func TestWriteSourceStatsCSV_EmptyList_OnlyHeader(t *testing.T) {
	var buf bytes.Buffer
	require.NoError(t, exportdata.WriteSourceStatsCSV(&buf, nil))
	require.Equal(t, "SourceIP,TotalCount,PassRate,Hostname,Service,FirstSeen,LastSeen\n", buf.String())
}
